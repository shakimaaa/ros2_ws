...
//   }
  ...
  // Plus operation: theta_plus_delta = NormalizeAngle(theta + delta)
  template <typename T>
  bool Plus(const T* theta, const T* delta, T* theta_plus_delta) const {
    *theta_plus_delta = NormalizeAngle(*theta + *delta);
    return true;
  }

  // Minus operation: delta = NormalizeAngle(theta2 - theta1)
  template <typename T>
  bool Minus(const T* theta2, const T* theta1, T* delta) const {
